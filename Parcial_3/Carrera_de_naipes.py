...
def crear_baraja() -> list[tuple[int, str, str]]:
    ...
    return [(valor, palo, f"{valor}{simbolo}") for palo, simbolo in palos.items() for valor in
            valores]  # para crear la baraja
...
def retroceso(caballos: dict[str, int], cartas_fuera: list[str]) -> None:
    ...
    """
    if all(pos > 0 for pos in
           caballos.values()) and cartas_fuera:  # Sí todos los caballos han avanzado y hay cartas jugadas
        primera_carta = cartas_fuera.pop(0)  # obtiene la primera carta jugada y la elimina de la lista
        palo_carta = next(
            (p for p, s in {'oros': '🟡', 'copas': '🍷', 'espadas': '⚔️', 'bastos': '🌿'}.items() if s in primera_carta),
            None)  # Obtiene el palo de la carta
...
def jugar_carrera() -> None:
    ...
    print(
        r"    ____      _      ____    ____    _____   ____       _        ____    _____     _   _      _      ___   ____    _____   ____   ")
    print(
        r"   / ___|    / \    |  _ \  |  _ \  | ____| |  _ \     / \      |  _ \  | ____|   | \ | |    / \    |_ _| |  _ \  | ____| / ___|  ")
    print(
        r"  | |       / _ \   | |_) | | |_) | |  _|   | |_) |   / _ \     | | | | |  _|     |  \| |   / _ \    | |  | |_) | |  _|   \___ \  ")
    print(
        r"  | |___   / ___ \  |  _ <  |  _ <  | |___  |  _ <   / ___ \    | |_| | | |___    | |\  |  / ___ \   | |  |  __/  | |___   ___) | ")
    print(
        r"   \____| /_/   \_\ |_| \_\ |_| \_\ |_____| |_| \_\ /_/   \_\   |____/  |_____|   |_| \_| /_/   \_\ |___| |_|     |_____| |____/  ")
    ...
    eleccion = input(
        "Apuesta por un caballo (oros, copas, espadas, bastos): ").strip().lower()  # Pide al usuario que elija un caballo
    while eleccion not in caballos:  # Mientras la elección no sea válida
        eleccion = input(
            "Opción inválida. Ingresa un palo válido (oros, copas, espadas, bastos): ").strip().lower()  # Pedirá al usuario que vuelva a elegir
